...
| `/projection-visual/`                         | 既定のoperator画面です。通常の自然会話を既存の `homeStore` / SpeakQueue へ渡し、アバター、調整可能な吹き出し、TTS、display stateを同じ表現経路で駆動します。operatorとStage Outputは同じeffect-host候補であり、ブラウザの排他lockを取得した一画面だけがintentを実行して相関receiptを返します。非ownerは成功receipt後だけ同じpresentationをミラーします。文字とpresentation Stopだけを隔離して確認する場合に限り、`requestMode=minimal-transient-text-v1`を明示します。 |
| `/projection-visual/?mode=stage-output&hud=0` | 操作UIを持たない正式な投影・配信面です。LauncherとDisplay Runtimeが使うcanonical output URLはこのままです。operatorと同じeffect-host leaseへ参加するため、この画面だけでも動き、両画面を開いた場合も実行・receipt ownerは一つです。                                                                                                                                                                                                                                    |
...
effect-host leaseにはsame-originのWeb Locksを使います。Web Locksを利用できない環境では
二重ownerになる代替実装へfall backせず、effect receiverをfail closedにします。これは
表示されたピクセルや物理投影の成功を証明するものではありません。

通常の `/`、`/projection-visual/?mode=operator`、`direct_send`、accepted prepared-sample
の各経路は、応答を `homeStore` / SpeakQueue へ渡してアバター発話と音声を駆動します。
`requestMode=minimal-transient-text-v1`を明示したoperatorと
`/projection-visual-minimal`だけが、strict bubbleとpresentation Stopに限定された
transient hook-local responseです。検証用の最小経路を通常のowner体験へ昇格させません。
...
- Node.js: 24.x
- npm: `>=11.16.0 <13`（プロジェクトの `allowScripts` ポリシーを強制するため）
...
- **ローカルAPIの公開範囲**: メッセージ送信、TTS、アップロードなどのAPIはローカル運用前提です。LAN公開やプロキシ配下で使う場合は、[セキュリティ運用メモ](./docs/security-notes.md)を確認してください。
