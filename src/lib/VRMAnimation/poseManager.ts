  ...
  async applyPose(
    ...
    if (isSequence) {
      const [poses, idleVrma] = await Promise.all([
        ...
        model.isIdleNeutralVisualTestMode()
          ? Promise.resolve(null)
          : loadVRMAnimation(buildUrl('/idle_loop.vrma')),
      ])
      if (
        poses.some((p) => !p) ||
        (!idleVrma && !model.isIdleNeutralVisualTestMode())
      )
        return
      ...
      let additiveAction: THREE.AnimationAction | undefined
      if (idleVrma) {
        ...
        additiveAction = model.mixer.clipAction(additiveClip)
      ...
      if (!pose || (!idleVrma && !model.isIdleNeutralVisualTestMode())) return
      if (requestId !== this.applyRequestId) return
