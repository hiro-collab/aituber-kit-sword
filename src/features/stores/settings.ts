...
interface Character {
  ...
  listeningPoseEnabled: boolean
  listeningPoseId: string
}
...
const getSelectedPresetIndexFromEnv = (): number => {
  const presetNumber = parseInt(
    process.env.NEXT_PUBLIC_SELECTED_PRESET_INDEX || '1',
    10
  )

  if (!Number.isFinite(presetNumber)) return 0
  return Math.min(Math.max(presetNumber, 1), 5) - 1
}
...
const getInitialValuesFromEnv = (): SettingsState => ({
  ...
  thoughtCoreSessionId:
    process.env.NEXT_PUBLIC_THOUGHT_CORE_SESSION_ID || 'aituber-kit',
  ...
  selectedPresetIndex: getSelectedPresetIndexFromEnv(),
  showAssistantText:
  ...
  commentTextColor: getCommentTextColor(
    process.env.NEXT_PUBLIC_COMMENT_TEXT_COLOR || DEFAULT_COMMENT_TEXT_COLOR
  ),
  commentTextSizePx: clampCommentTextSize(
    parseInt(
      process.env.NEXT_PUBLIC_COMMENT_TEXT_SIZE_PX ||
        `${DEFAULT_COMMENT_TEXT_SIZE_PX}`
    ) || DEFAULT_COMMENT_TEXT_SIZE_PX
  ),
  ...
  poseConfigs: [
    { id: 'listening', json: '/poses/finger_touch.json' },
  ...
  listeningPoseEnabled:
    process.env.NEXT_PUBLIC_LISTENING_POSE_ENABLED === 'true' || false,
  listeningPoseId: process.env.NEXT_PUBLIC_LISTENING_POSE_ID || 'listening',
...
const mergePersistedSettings = (
  ...
  if (systemCellAIService) {
    ...
    if (process.env.NEXT_PUBLIC_THOUGHT_CORE_BASE_URL) {
      mergedState.thoughtCoreUrl = process.env.NEXT_PUBLIC_THOUGHT_CORE_BASE_URL
    }
  }
  ...
  if (process.env.NEXT_PUBLIC_ALWAYS_OVERRIDE_WITH_ENV_VARIABLES === 'true') {
    ...
    if (systemCellAIService) {
      ...
      if (process.env.NEXT_PUBLIC_THOUGHT_CORE_BASE_URL) {
        envState.thoughtCoreUrl = process.env.NEXT_PUBLIC_THOUGHT_CORE_BASE_URL
      }
    }
    return envState
  }
  ...
  exclusivityMiddleware(
    persist(() => getInitialValuesFromEnv(), {
      ...
      partialize: (state) => ({
        ...
        commentTextColor: state.commentTextColor,
        commentTextSizePx: state.commentTextSizePx,
        ...
        listeningPoseEnabled: state.listeningPoseEnabled,
        listeningPoseId: state.listeningPoseId,
      }),
    })
  )
)
