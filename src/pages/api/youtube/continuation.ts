...
import { getTrustedRequestUrl } from '@/utils/serverUrlSecurity'
...
export default async function handler(
  ...
  if (!enforceLocalApiRequest(req, res, { feature: 'youtube/continuation' })) {
    return
  }
  ...
  // ローカルLLMのURL検証
  let trustedLocalLlmUrl = localLlmUrl
  if (isVercelLocalAIService(aiService) && aiService !== 'custom-api') {
    ...
    trustedLocalLlmUrl = getTrustedRequestUrl(localLlmUrl, {
      allowPath: true,
      allowedUrlsEnv: process.env.LOCAL_LLM_ALLOWED_URLS,
    })
    if (!trustedLocalLlmUrl) {
      return res.status(400).json({
        error: 'Invalid or untrusted Local LLM URL',
        errorCode: 'InvalidLocalLLMURL',
      })
    }
  }
  ...
  try {
    ...
    console.error('Error in youtube continuation API:', error)
