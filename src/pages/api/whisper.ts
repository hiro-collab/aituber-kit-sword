...
export default async function handler(
  ...
  if (!enforceLocalApiRequest(req, res, { feature: 'whisper' })) {
    return
  }
  ...
  try {
    ...
    if (audioFilePart.data.length > MAX_AUDIO_UPLOAD_BYTES) {
      return res.status(413).json({ error: 'Audio file is too large' })
    }
    if (
      audioFilePart.type &&
      !ALLOWED_AUDIO_TYPES.has(audioFilePart.type.toLowerCase())
    ) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (chunk) => {
      ...
      if (total > MAX_AUDIO_UPLOAD_BYTES) {
        reject(new Error('Audio request body is too large'))
        req.destroy()
      }
    })
