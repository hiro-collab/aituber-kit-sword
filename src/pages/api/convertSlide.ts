...
async function convertPdfToImages(pdfBuffer: Buffer): Promise<string[]> {
  ...
  if (pageCount > MAX_PDF_PAGES) {
    throw new Error(`PDF has too many pages. Max ${MAX_PDF_PAGES} pages.`)
  }
  const images: string[] = []
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!enforceLocalApiRequest(req, res, { feature: 'convert-slide' })) {
    return
  }
  ...
  const form = formidable({
    ...
    filter: (part) => PDF_MIME_TYPES.has(part.mimetype || ''),
  })
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const safeFolderName = validateSlideFolderName(
    ...
    const slideDir = resolveInsideDirectory(slidesDir, safeFolderName)
    const markdownPath = resolveInsideDirectory(slideDir, 'slides.md')
    const jsonPath = resolveInsideDirectory(slideDir, 'scripts.json')
...
function validateSlideFolderName(
