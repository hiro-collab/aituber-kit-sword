...
export default async function handler(
  ...
  if (!enforceLocalApiRequest(req, res, { feature: 'save-chat-log' })) {
    return
  }
  ...
  try {
    ...
    const requestedFileName = isNewFile
      ? newFileName
      : targetFileName || getLatestLogFile(logsDir) || newFileName
    const fileName = validateLogFileName(requestedFileName)

    if (!fileName) {
      return res.status(400).json({ message: 'Invalid targetFileName' })
    }

    const filePath = resolveInsideDirectory(logsDir, fileName)
...
function validateLogFileName(fileName: string | null): string | null {
  if (!fileName) return null
  const normalized = fileName.replace(/\\/g, '/')
  const baseName = path.basename(normalized)
  if (baseName !== normalized) return null
  if (baseName.includes('..')) return null
  if (!/^log_[A-Za-z0-9._-]+\.json$/.test(baseName)) return null
  return baseName
}
...
function resolveInsideDirectory(directory: string, filename: string): string {
  ...
  if (relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error('Resolved path is outside the target directory')
  }
