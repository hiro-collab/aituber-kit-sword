...
export default async function handler(
  ...
  if (!enforceLocalApiRequest(req, res, { feature: 'get-pngtuber-list' })) {
    return
  }

  const pngtuberDir = path.resolve(process.cwd(), 'public/pngtuber')
  ...
  try {
    ...
    for (const folder of folders) {
      ...
      const folderPath = resolveInsideDirectory(pngtuberDir, folder.name)
      ...
      // mouth/ ディレクトリを確認
      const mouthDir = resolveInsideDirectory(folderPath, 'mouth')
