...
export default async function handler(
  ...
  if (!enforceLocalApiRequest(req, res, { feature: 'update-slide-data' })) {
    return
  }
  ...
  const targetDir = resolveInsideDirectory(slidesDir, sanitizedSlideName)
  const scriptFilePath = resolveInsideDirectory(targetDir, 'scripts.json')
  const supplementFilePath = resolveInsideDirectory(targetDir, 'supplement.txt')
