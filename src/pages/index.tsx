...
const configuredThoughtCoreUrl =
...
const Home = () => {
  ...
  useEffect(() => {
    if (!configuredSystemCellAIService) return
    settingsStore.setState({
      selectAIService: configuredSystemCellAIService,
      ...(configuredThoughtCoreUrl
        ? { thoughtCoreUrl: configuredThoughtCoreUrl }
        : {}),
    })
  }, [])
  ...
  return (
    <div className="h-[100svh] bg-cover" style={backgroundStyle}>
      ...
      <Menu />
      <ModalImage />
      {controlOwner.isOwner && messageReceiverEnabled && <MessageReceiver />}
      <Toasts />
      {controlOwner.isOwner && (
        <>
          <WebSocketManager />
      ...
      <CharacterPresetMenu />
      <ImageOverlay />
      {controlOwner.isOwner && <PresenceManager />}
      <div className="absolute top-4 left-4 z-30">
        {controlOwner.isOwner && <IdleManager />}
      </div>
      <KioskOverlay />
    </div>
  )
