...
const allowExternalSystemPrompt =
  process.env.NEXT_PUBLIC_ALLOW_EXTERNAL_SYSTEM_PROMPT === 'true'
...
const SendMessage = () => {
  ...
  const handleSubmit = async (
    ...
    const body: RequestBody = {
      ...
      useCurrentSystemPrompt: allowExternalSystemPrompt
        ? useCurrentSystemPrompt
        : true,
      ...(allowExternalSystemPrompt && !useCurrentSystemPrompt
  ...
  const getTabCurlSample = () => {
    switch (activeTab) {
      ...
      case 'ai_generate':
        return allowExternalSystemPrompt
          ? `curl -X POST -H "Content-Type: application/json" -d '{"systemPrompt": "You are a helpful assistant.", "useCurrentSystemPrompt": false, "messages": ["この画像について説明してください。"], "image": "data:image/png;base64,..."}' '${baseUrl}/api/messages/?clientId=${clientId}&type=ai_generate'`
          : `curl -X POST -H "Content-Type: application/json" -d '{"messages": ["この画像について説明してください。"], "image": "data:image/png;base64,..."}' '${baseUrl}/api/messages/?clientId=${clientId}&type=ai_generate'`
      case 'user_input':
    ...
    <div className="flex flex-col items-center text-theme-default min-h-screen bg-theme py-8">
      <div className="w-full max-w-4xl px-4 md:px-8">
        ...
        <div className="mb-6 bg-white rounded-xl p-6 shadow-md">
          ...
          {activeTab === 'ai_generate' && allowExternalSystemPrompt && (
            <div className="mb-6">
