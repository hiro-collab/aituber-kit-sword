...
describe('/api/messages', () => {
  ...
  beforeEach(() => {
    jest.resetModules()
    delete process.env.ALLOW_EXTERNAL_SYSTEM_PROMPT
  ...
  describe('POST', () => {
    ...
    it.each([
      ['whitespace only', ['   '], 400, 'Messages must be non-empty strings'],
      [
        'object item',
        [{ text: 'hello' }],
        400,
      ...
      [
        'too many items',
        Array.from({ length: 21 }, (_, index) => `msg-${index}`),
        413,
        'Too many messages',
      ],
    ])(
      'should reject malformed messages payload: %s',
      (_label, messages, status, error) => {
        const req = createMockReq({
          method: 'POST',
          query: { clientId: 'client1' },
          body: { messages },
        ...
        expect(res._json).toEqual({ error })
      }
    )

    it('should reject prototype-looking client ids', () => {
      const req = createMockReq({
        method: 'POST',
        query: { clientId: '__proto__' },
        body: { messages: ['hello'] },
      })
      const res = createMockRes()
    ...
    it.each([
      ...
      ['whitespace', 'client one'],
      ['query punctuation', 'client?one'],
      ['too long', 'x'.repeat(129)],
    ])('should reject malformed client id: %s', (_label, clientId) => {
      const req = createMockReq({
        method: 'POST',
        query: { clientId } as unknown as NextApiRequest['query'],
        body: { messages: ['hello'] },
      })
      const res = createMockRes()
    ...
    it('should reject invalid message types', () => {
      const req = createMockReq({
        method: 'POST',
        query: { clientId: 'client1', type: 'unknown' },
        body: { messages: ['hello'] },
      })
      const res = createMockRes()
      ...
      expect(res._status).toBe(400)
      expect(res._json).toEqual({ error: 'Invalid message type' })
    })
    ...
    it('should ignore external systemPrompt by default', () => {
      handler(
        createMockReq({
          method: 'POST',
          query: { clientId: 'client1' },
      ...
      expect(messages[0]).toEqual(
        expect.objectContaining({
          ...
          useCurrentSystemPrompt: undefined,
        })
      )
    })

    it('should include systemPrompt only when explicitly allowed', () => {
      jest.resetModules()
    ...
    it.each([
      ['number', 123, 400, 'Image is not a string'],
      [
        'object',
      ...
      ['plain base64', 'AAAA', 400, 'Image must be a base64 image data URI'],
      [
        'wrong mime',
        'data:text/plain;base64,AAAA',
        400,
        'Image must be a base64 image data URI',
      ],
    ])(
      'should reject malformed image payload: %s',
      (_label, image, status, error) => {
        const req = createMockReq({
          method: 'POST',
          query: { clientId: 'client1', type: 'ai_generate' },
          body: {
    ...
    it('should normalize null image to omitted image', () => {
      handler(
        createMockReq({
          method: 'POST',
          query: { clientId: 'client1', type: 'ai_generate' },
          body: {
            messages: ['describe without image'],
            image: null,
          },
        }),
        createMockRes()
      ...
      expect(messages[0]).toEqual(
        expect.objectContaining({
          message: 'describe without image',
          image: undefined,
        })
      )
    })
