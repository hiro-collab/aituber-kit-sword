...
describe('/api/tts-aivisspeech', () => {
  ...
  it('should use loopback custom serverUrl when provided', async () => {
    ...
    expect(mockAxiosPost.mock.calls[0][0]).toContain('http://127.0.0.1:10102')
  })
  ...
  it('should reject untrusted remote serverUrl', async () => {
    const req = createMockReq({
      body: {
        ...
        pitch: 0,
        intonationScale: 1,
        serverUrl: 'http://attacker.example:10101',
      },
    })
    const res = createMockRes()
    ...
    expect(res._json).toEqual({ error: 'Invalid AivisSpeech server URL' })
