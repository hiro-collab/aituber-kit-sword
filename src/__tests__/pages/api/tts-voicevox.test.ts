...
describe('/api/tts-voicevox', () => {
  ...
  it('should use loopback custom serverUrl when provided', async () => {
    const mockPipe = jest.fn()
    mockAxiosPost
      .mockResolvedValueOnce({ data: {} })
      .mockResolvedValueOnce({ data: { pipe: mockPipe } })
    ...
    const req = createMockReq({
      body: {
        ...
        pitch: 0,
        intonation: 1,
        serverUrl: 'http://127.0.0.1:50022',
      },
    })
    const res = createMockRes()
    ...
    expect(mockAxiosPost.mock.calls[0][0]).toContain('http://127.0.0.1:50022')
  })

  it('should reject untrusted remote serverUrl', async () => {
    const req = createMockReq({
      body: {
        text: 'test',
        speaker: 1,
        ...
        pitch: 0,
        intonation: 1,
        serverUrl: 'http://attacker.example:8080',
      },
    })
    const res = createMockRes()
    ...
    expect(res._json).toEqual({ error: 'Invalid VOICEVOX server URL' })
