...
describe('worker-2 projection visual organ contract', () => {
  ...
  it('keeps normal HUD focused on Thought Core while legacy Dify stays developer-only', () => {
    ...
    expect(source).toContain('actionReadinessIndicators')
    expect(source).toContain('environmentReadinessValueLabel')
    expect(source).toContain('environmentActionState')
    expect(source).toContain('environmentActionDetailLabel')
    expect(source).toContain('Home action readiness:')
    expect(source).toContain('Testable')
