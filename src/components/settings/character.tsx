    ...
    <>
      ...
      <div className="">
        ...
        {modelType === 'vrm' && (
          <div className="my-6">
            <div className="text-xl font-bold mb-4">
              {isJa ? '聞き取り中ポーズ' : 'Listening Pose'}
            </div>
            <div className="mb-4 text-sm">
              {isJa
                ? 'マイクが聞き取り中のあいだ、指定したVRMポーズを表示します。'
                : 'Shows the selected VRM pose while the microphone is listening.'}
            </div>
            <ToggleSwitch
              enabled={listeningPoseEnabled}
              onChange={(v) =>
                settingsStore.setState({ listeningPoseEnabled: v })
              }
            />
            {listeningPoseEnabled && (
              <div className="mt-4">
                <div className="text-sm font-bold mb-2">
                  {isJa ? '聞き取り中に表示するポーズ' : 'Pose while listening'}
                </div>
                <select
                  className="text-ellipsis px-4 py-2 w-full sm:w-col-span-2 bg-white hover:bg-white-hover rounded-lg"
                  value={listeningPoseId}
                  onChange={(e) =>
                    settingsStore.setState({ listeningPoseId: e.target.value })
                  }
                >
                  {poseConfigs.map((pose) => (
                    <option key={pose.id} value={pose.id}>
