      ...
      case 'thought-core':
        return (
          <>
            <div className="my-6">
              <div className="my-2 text-sm whitespace-pre-wrap">
                Local Thought Core runtime. Legacy compatibility providers can
                still be configured separately.
              </div>
              <div className="my-4 text-xl font-bold">Thought Core API URL</div>
              <input
                className="text-ellipsis px-4 py-2 w-full sm:w-col-span-2 bg-white hover:bg-white-hover rounded-lg"
                type="text"
                placeholder="http://127.0.0.1:18888"
                value={state.thoughtCoreUrl}
                onChange={(e) =>
                  settingsStore.setState({ thoughtCoreUrl: e.target.value })
                }
              />
            </div>
            <div className="my-6">
              <div className="my-4 text-xl font-bold">Thought Core session</div>
              <input
                className="text-ellipsis px-4 py-2 w-full sm:w-col-span-2 bg-white hover:bg-white-hover rounded-lg"
                type="text"
                placeholder="aituber-kit"
                value={state.thoughtCoreSessionId}
                onChange={(e) =>
                  settingsStore.setState({
                    thoughtCoreSessionId: e.target.value,
                  })
                }
              />
            </div>
